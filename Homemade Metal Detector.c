...
#define LCD_D6 LATBbits.LATB15
#define LCD_D7 LATBbits.LATB13
...
static const unsigned long int wav_index[]={
    0x00002d, // small-non
    0x004532, // ferrous metal
    0x008641, // large non-ferrous metal
    0x012345, // small ferrous metal
    0x01ad07, // large
    0x01f04f, // ferrous metal
    0x022f7f, // ... 
    0x027386
};
...
//***************CALIBRATION VARIABLES*******************************//
	int EmptyCountUpLim = 56255; //Count without any metal near inductor upper limit
	int EmptyCountLowLim = 56200; //Count without any metal near inductor lower limit
	int SmallNonFerrous = 56170;//Upper limit of small non ferrous metal, below this is large nonferrous
	int SmallFerrous =56300;//Lower limit of small ferrous metal, above this is large ferrous
//*******************************************************************//

	int Verification;	//Verifies changes in count
	int VerificationNull; 
	float T, f;
    ...
    while(1)
    {
		...
		if(count>=EmptyCountLowLim & count<=EmptyCountUpLim)//If count returns to standard count for 400ms, no metallic reading
		{
			VerificationNull+=1;
				if(VerificationNull=2)
				{
					Verification=0;
					VerificationNull=0;
			}
		...
		if(Verification==0)
		{
			LATBbits.LATB12 = 1; // Kill led on RB12
			printf("No metal detected");
					sprintf(buff, "No Metal");
  					LCDprint(buff, 1, 1);
...
//Non ferrous metal
		if(count<EmptyCountLowLim)
		{
			Verification+=1; //Verification is a counter which requires 800ms of a count before changing metal reading

				
			if (Verification>3)// Verification *200ms = time required for a metallic reading
			{
				LATBbits.LATB12 = 0; // Turn on led on RB12
				
				if(count>=SmallNonFerrous)
					{
					printf(" small Non-Ferrous metal   ");
					sprintf(buff, "Small Non-");
  					LCDprint(buff, 1, 1);
 					sprintf(buff, "Ferrous Metal");
  					LCDprint(buff, 2, 1);

					index = 0;

    				start = wav_index[index];
    				nbytes = wav_index[index+1]-wav_index[index];
					...
					printf(" Large Non-Ferrous metal   ");
					index = 2;
					sprintf(buff, "Large Non-");
  					LCDprint(buff, 1, 1);
 					sprintf(buff, "Ferrous Metal");
  					LCDprint(buff, 2, 1);
...
//Ferrous metal
		if(count>EmptyCountUpLim)
		{
			Verification+=1; //Verification is a counter which requires 800ms of a count before changing metal reading
		
		
			if (Verification>2)// Verification *200ms = time required for a metallic reading
			{
				LATBbits.LATB12 = 0; // Turn on LED
				
				if(count<=SmallFerrous){
				printf(" small Ferrous metal   ");
				sprintf(buff, "Small Ferrous");
  				LCDprint(buff, 1, 1);
 				sprintf(buff, "Metal");
  				LCDprint(buff, 2, 1);
				...
				while(play_flag);
					...
					printf(" large Ferrous metal   ");
					sprintf(buff, "Large Ferrous");
  					LCDprint(buff, 1, 1);
 					sprintf(buff, "Metal");
  					LCDprint(buff, 2, 1);
